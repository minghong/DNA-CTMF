...
while(tes<1000000):
    ...
    out=open(str(error)+"_time_all.txt","a")

    selected_sequence = random.choices(map_rule, k=30)
    
    sequence="".join(sequence_error(selected_sequence, error));
    ...
    try:
        ...
        #定位错误位置
            ...
            si.alarm(0)
            out.write(str(time.time()-start)+"\n")


            out.close()
            correct=merge_sequence(xx,yy,right_merge[0][0],wrong_merge[0][0])
            five_base_group=split_string_by_length(correct,5)    
            correct_pixel=[]
